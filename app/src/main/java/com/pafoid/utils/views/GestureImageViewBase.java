...
import com.pafoid.utils.BuildConfig;

public abstract class GestureImageViewBase extends AppCompatImageView {
    ...
    /**
     * Use this to change the {@link GestureImageViewBase#setDisplayType(DisplayType)} of
     * this View
     *
     * @author alessandro
     */
    ...
    public GestureImageViewBase(Context context) {
        this(context, null);
    }

    public GestureImageViewBase(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public GestureImageViewBase(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setImageBitmap(final Bitmap bitmap, Matrix matrix, float minZoom, float maxZoom) {
        if (bitmap != null) {
            setImageDrawable(new BitmapDrawable(bitmap), matrix, minZoom, maxZoom);
        } else {
    ...
    protected float computeMaxZoom() {
        ...
        //Always calculate in portrait mode
        float fw = mBitmapRect.width() / Math.min(mViewPort.width(), mViewPort.height());
        float fh = mBitmapRect.height() / Math.max(mViewPort.width(), mViewPort.height());
